...
public class UpdateOrderServiceTest extends AbstractUnitTest implements OCMUnitTest {
  ...
  public void withUpdateOrderServiceCallOrderIsUpdatedSuccessfully() {
    ...
    ////    assertThat(response.getListOfOrderItems().size(), is(1)); // Assert that updated order
    // has items
    ////    assertThat(response.getListOfOrderItems().get(0).getItemName(), is("Pizzunia Product
    // UPDATE"));
    //
