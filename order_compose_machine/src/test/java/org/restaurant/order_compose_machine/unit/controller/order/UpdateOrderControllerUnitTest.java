...
public class UpdateOrderControllerUnitTest extends AbstractUnitTest implements OCMUnitTest {
    ...
    public void withProperOrderPayloadOrderUpdatedWithHttp200() {
        ...
        ResponseEntity<ApiResponse<OrderDto>> responseOrder = orderController.updateOrder(updatedOrderDto, orderId);
        verify(orderService).updateOrder(updatedOrderDto, orderId);
        assertThat(responseOrder.getBody().getData().getListOfOrderItems().stream().findFirst().get().getItemName(), is("Pizzunia Product UPDATE"));
    }
}
